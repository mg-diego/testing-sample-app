...
TAB_CONFIG = [
    (get_translation('menu.homepage'), None),
    (get_translation('menu.catalog'), Permissions.ACCESS_CATALOG_MANAGEMENT),
    (get_translation('menu.userManagement'), Permissions.ACCESS_USER_MANAGEMENT),
    (get_translation('menu.language'), Permissions.SET_LANGUAGE),
    (get_translation('menu.logout'), None),
]

def get_headers():
    return  { "Authorization": "Bearer " + st.session_state["access_token"] }

def show_toast(response: requests.Response, custom_message, custom_error_message):
    response_json = response.json()
    if response.ok:
        st.session_state.toast['text'] = f"{custom_message}"
        st.session_state.toast['icon'] = "✅"
    else:
        translation = get_translation(f"errors.{response_json['detail']}")
        st.session_state.toast['text'] =  translation if '%s' not in translation else translation % custom_error_message
        st.session_state.toast['icon'] = "🚨"
...
def create_catalog():
    ...
    if st.button("Create"):
        ...
        show_toast(response, get_translation('catalog.toast.create') % name, name if name!= None else '')
        st.rerun()
...
def edit_catalog(catalog_id, catalog_name, catalog_description):
    ...
    if st.button("Save changes"):
        ...
        show_toast(response, get_translation('catalog.toast.edit') % catalog_id, name if name!= None else '')
        st.rerun()
...
def show_web():
    ...
    for label, permission in TAB_CONFIG:
        if permission is None or verify_permission(permission, st.session_state["access_token"]):
            allowed_tabs.append(label)
            tab_logic_map[label] = permission  # Store permission logic if needed

    tabs = st.tabs(allowed_tabs)

    for tab_label, tab_obj in zip(allowed_tabs, tabs):
        with tab_obj:
            if tab_label == get_translation('menu.homepage'):
            ...
            elif tab_label == get_translation('menu.catalog'):
            ...
            elif tab_label == get_translation('menu.userManagement'):
                        ...
                        with st.form("user_form", clear_on_submit=True):
                            ...
                            if submitted:
                                ...
                                if error_message != "":
                                    ...
                                    show_toast(create_response, get_translation('userManagement.createUser.toast.success') % username, username)
                                    st.rerun()
            ...
            elif tab_label == get_translation('menu.language'):
                ...
                with st.form("Select language"):
                    ...
                    if submitted:                
                        ...
                        show_toast(response, get_translation("language.toast.success") % get_language_code(selected_language, languages).upper(), None)
                        st.rerun()

            elif tab_label == get_translation('menu.logout'):
