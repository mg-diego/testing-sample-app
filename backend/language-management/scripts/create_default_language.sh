...
CREATE_TABLE_QUERY="CREATE TABLE IF NOT EXISTS public.language (
    id SERIAL PRIMARY KEY,
    language VARCHAR(100) NOT NULL
);"

# Comando SQL para verificar si el usuario ya existe
SQL_QUERY="SELECT 1 FROM public.language WHERE language='en';"
...
if [ -z "$USER_EXISTS" ]; then
    echo "El idioa. Creando usuario admin..."
